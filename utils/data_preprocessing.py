...
def load_data(dataset, base_dir, output_folder, json_folder):
    ...
    #################################################################################
    #
    #       Benchmark dataset(SMD, SMAP, MSL, SWaT, WADI)
    #
    #################################################################################
    #################################################################################
    if dataset == 'SMD':
...
#                 dump(data, file)
    ...
    #################################################################################
    #
    #       Semics : processing (csv file -> npy file)
    #
    #################################################################################
    #################################################################################
    elif dataset == 'semics':      
        folder_path = os.path.join(options.data_dir , 'semics_data')
        ...
        for j in range(len(folder_list)):
            data = pd.read_csv(os.path.join(folder_path, folder_list[j]))

            normal_data = data[data['Anomaly_ID'] == 0].iloc[:,3:6].to_numpy()
            # train_files.append(normal_data[:round(len(normal_data)/2)])
            train_files.append(normal_data)

            anormal_label = data['Anomaly_ID'].to_numpy().astype(int)
            print(anormal_label.shape)
            anormal_data = data.iloc[:,3:6].to_numpy()
            print(anormal_data.shape)
            
            # test_files.append(anormal_data[:round(len(data)/2)])
            test_files.append(anormal_data)
            # label_files.append(anormal_label[:round(len(data)/2)])
            label_files.append(anormal_label)
            file_length.append(len(label_files[-1]))
    ...
    elif dataset == 'semics2':      
        folder_path = os.path.join(options.data_dir , 'semics_data2')
        folder_list = os.listdir(folder_path)
        print(folder_list)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset", required=True, default='semics', type=str,
                        help="Name of dataset; SMD/SMAP/MSL/SWaT/WADI")
    parser.add_argument("--data_dir", required=None, default = './data', type=str,
    ...
    datasets = ['SMD', 'SMAP', 'MSL', 'SWaT', 'WADI', 'semics', 'semics2']
