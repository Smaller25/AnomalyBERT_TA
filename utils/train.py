...
def main(options):
    ...
    if data_division == 'total':
        ...
        # semics-dataset division
        if options.dataset == "semics-t" or "semics-d" or "semics" or "semics-all-new" or "semics-total-I":
            start = 0
            end = len(test_data)
            step = 100000
        ...
        for j, rep, tar, leng, typ, dim_num, dim_cat in zip(range(n_batch), replacing_index, target_index, replacing_lengths,
            ...
            if leng > 0:
                ...
                # Uniform replacing
                    ...
                    _x[target_column_numerical] = torch.rand(rep_len_num, 1, device=device)/4
...
##### semics-t 기준 #####
        # st_idx = [0,12000,18000,20000,32000,35000,43000]
        # ed_idx = [3000,13000,19000,23000,33000,37000,44000]
        # for j in range(len(st_idx)):
        #     start_idx = st_idx[j]
        #     end_idx = ed_idx[j]
        #     label = x_anomaly.cpu().numpy().reshape(-1)[start_idx:end_idx].astype(bool)
        #     print(len(label))
        #     # print(label)

        #     fig,ax = plt.subplots()
        #     plotting_data = z.cpu().reshape(-1, 3)[start_idx:end_idx]
        #     # print(len(plotting_data))
        #     print(np.arange(end_idx-start_idx)[label])
        
        #     ax.plot(plotting_data)
        #     ax.scatter(np.arange(end_idx-start_idx)[label], x_anomaly.cpu().numpy().reshape(-1)[start_idx:end_idx][label], c='r', s=1, alpha=0.8)
        #     fig.savefig(f'synt_data_iteration_{i}_{j}.png')
