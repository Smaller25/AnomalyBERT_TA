...
def main(options):
    ...
    ####################################################################################################
    #
    #       replacing data : option(argument 중에서 replacing_data에서 파일 경로 수정하면 됨)
    #
    ####################################################################################################
    ####################################################################################################
    replacing_data = train_data if options.replacing_data == None\
                     else np.load(options.rep_data, allow_pickle=True).copy().astype(np.float32)
    ...
    if data_division == 'total':
        ...
        # semics-dataset division : 한번에 너무 많이 들어가면 메모리가 터질 수 있어서 division 나눠서 들어가게끔 함
        if options.dataset == "semics" or "semics2":
            start = 0
            end = len(test_data)
            step = 100000
    ...
    data_seq_len = n_features * options.patch_size    # 2048
    # print(data_seq_len)       # 2048 (512 * 4)
    # print(d_data)    # 3
    ...
    valid_index_list = np.arange(len(train_data) - data_seq_len)      # len(train_data) - 2048
...
#         train_loss = lambda pred, gt: -(gt * torch.log(pred + 1e-8) + (1 - gt.bool().float()) * torch.log(1 - pred + 1e-8)).mean()
    ...
    # (1) Replaced data length table : anomaly length
    # 어떤 길이로 구간을 정해서 anomaly 생성할 것인지 지정하는 table을 생성. 이 과정에서 options.replacing_rate_max를 지정하면 그 길이만큼 바뀜
    # 지금은 일단 전체 data seq 길이에 따라 최대 anomaly 길이가 달라지도록 설정되어있지만, 이 부분을 고정값으로 50, 300 이렇게 지정해도 됨 (replacing_len_max 부분)
    ...
    # (2) Anomaly Synthesis : hyperparameter - type 지정
    # 생성 방법 : option에서 받아온 soft_replacing, uniform_replacing, peak_noising, length_replacing의 합은 1이 되어야 함.
    # 그래서 그 값들만큼 0,1 사이의 파이를 나눠가지고, 아래에서 typ이라는 값을 만들때 (0,1) 사이에서 random하게 값을 뽑아서 어느 파이에 속하는지 확인해서 
    # 그 방식대로 anomaly 생성함
    ############################################################################
    # 0 ---- length_r ---- peak_r ----- uniform_r ---- soft_r -------- 1
    ############################################################################
    # white_p : 위와 상관없이 option에서 받아옴
    # typ : random으로 0 ~ 1사이 값 생성 -> 어디에 있는지 체크해서 anomaly 생성
    ...
    for i in range(options.initial_iter, max_iters):
        # 확인용
        # print(f"soft_replacing_prob: {soft_replacing_prob}")
        # print(f"uniform_replacing_prob: {uniform_replacing_prob}")
        # print(f"peak_noising_prob: {peak_noising_prob}")
        # print(f"length_adjusting_prob: f{length_adjusting_prob}")
        # print(f"white_noising_prob: {white_noising_prob}")
        ...
        # Replacing types and dimensions : (0,1) 사이의 값을 uniform하게 random 선택하여 synthetics anomaly 결정
...
#         replacing_dim[categorical_column] = replacing_dim_categorical
        ...
        for j, rep, tar, leng, typ, dim_num, dim_cat in zip(range(n_batch), replacing_index, target_index, replacing_lengths,
            ...
            if leng > 0:
                ...
                ###################################################################
                #
                # (1) External interval replacing : replacing_data에서 불러옴
                #
                ###################################################################
                ###################################################################
                if typ > soft_replacing_prob:
                    typ_name = 'soft_replacing from external data'
                ...
                ###################################################################
                #
                # (2) Uniform replacing : 일정한 값으로 대체
                #
                ###################################################################
                ###################################################################
                elif typ > uniform_replacing_prob:
                    # scale 맞추기 위해 2.5로 나눔-> scale : (0~0.4)
                    # I_undergain 위해서는 이 값 역시 수정할 필요가 있음 -> (0 ~ 0.25)
                    uniform_value = torch.rand(rep_len_num, 1, device=device)/4
                    typ_name = f'uniform_replacing_prob_{uniform_value}'
                    _x[target_column_numerical] = uniform_value
#                     _x[target_column_categorical] = torch.randint(0, 2, size=(rep_len_cat, 1), device=device).float()
                ...
                ###################################################################
                #
                # (3) Peak noising : 특정 위치만 peak 생성 (매우 작은 anomaly 구간)
                #
                ###################################################################
                ###################################################################
                    ...
                    typ_name = f'peak_{target_column_numerical}_{peak_value}'
...
#                     _x[target_column_categorical, peak_index] = peak_value
                ...
                ###################################################################
                #
                # (4) Length adjusting (only for bce loss)
                #
                ###################################################################
                ###################################################################
                elif typ > length_adjusting_prob:
                    typ_name = 'length_adjusting'
                    # Lengthening
                ...
                ###################################################################
                #
                # Plotting : Synthetic analysis : 필요한 경우에 아래 부분 documentation 풀어서 하면 됨 
                #
                ###################################################################
                ###################################################################
                # # Plot 
                # fig, ax = plt.subplots()
                # label = x_anomaly[j].cpu().numpy().reshape(-1).astype(bool)
                # ax.plot(torch.tensor(x[j]).cpu().reshape(-1, 3))
                # ax.scatter(np.arange(0, len(x[j]))[label], x_anomaly[j].cpu().numpy().reshape(-1)[label], c='r', s=1, alpha=0.8)
                # ax.set_title(f"Synthetic data, portion_{j}: {typ_name}")
                # ax.legend(['x','y','z'])
                # fig.savefig(f'./synthetic_ex/window_iteration_part_{j}.png')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", default='SMAP', type=str, help='SMAP/MSL/SMD/SWaT/WADI/semics')
    parser.add_argument("--replacing_data", default=None, type=str, help='external data for soft replacement; None(default)/semics')
    ...
    parser.add_argument("--rep_data", default = './data/processed/semics_replacing_under.npy', type=str, help = 'real anomaly data for supervised learning.')
    ...
    parser.add_argument("--length_adjusting", default=0.1, type=float, help='probability for length adjustment')
    parser.add_argument("--white_noising", default=0.2, type=float, help='probability for white noise (deprecated)')
